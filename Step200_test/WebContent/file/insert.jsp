...
<%
	/*
		...
		commons fileupload 검색해서 commons-fileupload 파일 다운로드 해서
		WEB-INF/lib/ 폴더 안에 넣으면 아래와 같이 파일 업로드 처리를 할수 있다.
	*/
	
	//업로드 설정
	final int MEMORY_THRESHOLD   = 1024 * 1024 * 3;  // 3MB
	final int MAX_FILE_SIZE      = 1024 * 1024 * 40; // 40MB 1.상황에 따라 결정
    ...
    String uploadPath = application.getRealPath("/upload"); // "2. /upload" 보통 이렇게 씀
    //전송된 파라미터의 한글 인코딩 설정 
    ...
    try {
        ...
        if (formItems != null && formItems.size() > 0) {
        	// 3. 파일 정보를 담을 Dto 객체 생성
        	FileDto dto=new FileDto(); // 상황에 따라서
